...
class OpportuniteController extends Controller
{
     ...
     public function filtrer_statut_maTeam(Request $request)
    {
        ...
        $serachCom = $request->get('serachCom');
        $serachpays = $request->get('serachpays');
        $searchDo = $request->get('searchDo');
        $commercial = Commerciau::where('user_id', Auth::user()->id)->first();
        $opportunite = DB::table('opportunites')->where('entreprise_client_id', $commercial->entreprise_client_id)->where('commercial_id','like', '%'.$serachCom.'%')
        ->where('pays_id','like', '%'.$serachpays.'%')->where('statut','like', '%'.$search.'%')->where('origine_id','like', '%'.$searchOr.'%')->where('domaine_id','like', '%'.$searchDo.'%')
        ->whereIn('commercial_id', [$serachCom])->orWhereIn('pays_id', [$serachpays])->orWhereIn('statut', [$search])->orWhereIn('origine_id', [$searchOr])->orWhereIn('domaine_id', [$searchDo])->paginate(10000);
        return view('suiviSortieTerrain.opportunite_maTeam', compact('opportunite'));
    }
...
$venteMontantss = DB::table('ventes')->whereMonth('created_at', $mois)->whereYear('created_at', $annee)->sum('montant');
  ...
  public function appel_offre_edit($id)
    ...
    {
        $commercial = Commerciau::where('user_id', Auth::user()->id)->first();
        $opportunite = DB::table('opportunites')->where('commercial_id', $commercial->id)->OrderBy('probabilite', 'desc')->get();
        $opportunite_backup = DB::table('opportunites')->where('commercial_backup', $commercial->id)->OrderBy('probabilite', 'desc')->get();
        return view('suiviSortieTerrain.suivi_opportunite', compact('opportunite', 'opportunite_backup'));
    }
    
     public function marcher_suivi_opportunites()
    {
        $commercial = Commerciau::where('user_id', Auth::user()->id)->first();
        $opportunite = DB::table('opportunites')->where('commercial_id', $commercial->id)->OrderBy('probabilite', 'desc')->get();
        $opportunite_backup = DB::table('opportunites')->where('commercial_backup', $commercial->id)->OrderBy('probabilite', 'desc')->get();
        return view('suiviSortieTerrain.marcher_suivi_opportunite', compact('opportunite', 'opportunite_backup'));
    }
    ...
    public function filtrer_opportunite(Request $request)
    {
        ...
        $searchDo = $request->get('searchDo');
        $commercial = Commerciau::where('user_id', Auth::user()->id)->first();
        $opportunite = DB::table('opportunites')->where('commercial_id', $commercial->id)->where('statut','like', '%'.$search.'%')->where('origine_id','like', '%'.$searchOr.'%')
        ->where('domaine_id','like', '%'.$searchDo.'%')->paginate(10000);
        $opportunite_backup = DB::table('opportunites')->where('commercial_backup', $commercial->id)->where('statut','like', '%'.$search.'%')->where('origine_id','like', '%'.$searchOr.'%')
        ->where('domaine_id','like', '%'.$searchDo.'%')->OrderBy('probabilite', 'desc')->paginate(10000);
        return view('suiviSortieTerrain.suivi_opportunite', compact('opportunite', 'opportunite_backup'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->file('notes')){
           ...
           $notes->move(public_path().'/notes/', $file_name);
                ...
                $opportunite->commercial_backup = $request->get('commercial_backup');
                $opportunite->origine_id = $request->get('origine_id');
                    // $opportunite->domaine_id = $request->get('domaine_id')
                    // ->join('commerciaus', 'domaine.id');
                $opportunite->deadline = $request->get('deadline');
