...
class OpportuniteController extends Controller
{
      ...
      public function top_commerciaux_detail()
    {
        ...
        ->whereMonth('ventes.created_at', '>=', 04)
  ...
  public function appel_offre_edit($id)
    ...
    public function ajout_rv_op($id)
    {
     ...
     public function spontane_lister()
    {
        $commercial = Commerciau::where('user_id', Auth::user()->id)->first();
        $opportunite = DB::table('opportunites')->where('commercial_id', $commercial->id)->OrderBy('probabilite', 'desc')->paginate(22);
        $opportunite_backup = DB::table('opportunites')->where('commercial_backup', $commercial->id)->OrderBy('probabilite', 'desc')->paginate(22);
        return view('suiviSortieTerrain.spontane_lister', compact('opportunite', 'opportunite_backup'));
    }
    
    public function create_spontane()
    {
        //
        ...
        return view('suiviSortieTerrain.create_spontane',compact('prospect','backup', 'statut', 'origine'));

    }
    
     public function spontane_store(Request $request)
    {
        //
               ...
               // Mail::to('fallou.g@illimitis.com')->send(new MailBizdev($opportunite,$commercial_id));
                
                return redirect('/spontane-lister')->with(['message' => $message, 'ajout_action' => $ajout_action, 'ajout_rv' => $ajout_rv]);
    }
