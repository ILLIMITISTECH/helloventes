...
class PerformanceSemaine extends Command
    {
    /**
    ...
    * @var string
    */
    protected $signature = "performance:semaine";
         ...
         foreach($commerciale as $commercial){
             ...
             $pourcentage_visite_mois = $visite_ajouter * (100) / ($commercial->objectif_visite);
                 ...
                 $pourcentage_visite_mois = 0;
                   ...
                   $perfor_objectV = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('realisation_vente');
                   $perfor_objectC = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('realisation_contact');
                   $perfor_objectD = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('realisation_demo');
                   $perfor_objectVisite = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('realisation_visite');

                   $perfor_objectVp = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('perfo_vente');
                   $perfor_objectCp = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('perfo_contact');
                   $perfor_objectDp = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('perfo_demo');
                   $perfor_objectDVisite = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->sum('perfo_visite');
          ...
          $perfor_object = DB::table('performances')->where('commercial_id', $commercial->id)->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->first();
           ...
           'objectif_vente' => intval($obj_moi_com),
            ...
            $perfor_reaVenteGlobal = DB::table('performances')->whereMonth('created_at', $mois)->sum('realisation_vente');
            $perfor_objectVenteGlobal = DB::table('performances')->whereMonth('created_at', $mois)->sum('objectif_vente');
            
            $perfor_objectVenteGlobalc = DB::table('performances')->whereMonth('created_at', $mois)->sum('realisation_contact');
            $perfor_reaVenteGlobalC = DB::table('performances')->whereMonth('created_at', $mois)->sum('objectif_contact');
            
            $perfor_objectVenteGlobalVisite = DB::table('performances')->whereMonth('created_at', $mois)->sum('realisation_visite');
            $perfor_reaVenteGlobalVisite = DB::table('performances')->whereMonth('created_at', $mois)->sum('objectif_visite');
            
            $perfor_objectVenteGlobalD = DB::table('performances')->whereMonth('created_at', $mois)->sum('realisation_demo');
            $perfor_reaVenteGlobalD = DB::table('performances')->whereMonth('created_at', $mois)->sum('objectif_demo');
            
            $perfor_reaVenteg = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('realisation_vente');
            $perfor_objectVenteg = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('objectif_vente');
            
            $perfor_objectVentegReCont = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('realisation_contact');
            $perfor_objectVenteGlobalObCont = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('objectif_contact');
            
            $perfor_objectVentegReVisite = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('realisation_visite');
            $perfor_objectVenteGlobalObVisite = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('objectif_visite');
            
            $perfor_objectVentegDem = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('realisation_demo');
            $perfor_objectVenteGlobalDem = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('objectif_demo');
            
            $perfor_objectVenteGlobalPerfov = DB::table('performances')->whereMonth('created_at', $mois)->sum('perfo_vente');
            $perfor_objectVenteGlobalPerfod = DB::table('performances')->whereMonth('created_at', $mois)->sum('perfo_demo');
            $perfor_objectVenteGlobalPerfoc = DB::table('performances')->whereMonth('created_at', $mois)->sum('perfo_contact');
            $perfor_objectVenteGlobalPerfovisites = DB::table('performances')->whereMonth('created_at', $mois)->sum('perfo_visite');

            $perfor_objectVenteGlobalPerfovG = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('perfo_vente');
            $perfor_objectVenteGlobalPerfodG = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('perfo_demo');
            $perfor_objectVenteGlobalPerfocG = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('perfo_contact');
            $perfor_objectVenteGlobalPerfovisite = DB::table('performance_globales')->whereMonth('created_at', $mois)->sum('perfo_visite');
            ...
            $perfor_object_g = DB::table('performance_globales')->whereMonth('created_at', $mois)->orderBy('created_at', 'desc')->first();
            if($perfor_object_g == null)
            {
             DB::table('performance_globales')
            ->insert([
            'semaine' => 1,
