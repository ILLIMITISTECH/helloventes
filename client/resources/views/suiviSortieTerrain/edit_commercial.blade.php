       ...
       <a data-toggle="tooltip" title="Retour" style="width:90px; margin-left:30px"  type="button" id="PopoverCustomT-1" class="nm" href="javascript:history.go(-1)" >
                ...
                <div class="container px-6 mx-auto grid">
                    <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
                      @php $comer = DB::table('commerciaus')->where('id', $com->commercial_id)->first(); @endphp
                        @if($comer)
                       Modifier les informations de : {{$comer->prenom}} {{$comer->nom}}
                       @else
                    ...
                    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                        <!-- Invalid input -->
                        <form action="{{route('commerciaul.update', $com->id)}}" method="post" class="dropzone dropzone-custom needsclick add-professors" id="demo1-upload" enctype="multipart/form-data">
                                                     @csrf
                <input name="commercial_id" type="hidden"  value="{{$com->commercial_id}}"/>
                        ...
                        @php $commission = DB::table('parametres')->where('commission', $com->commission_p)->first(); @endphp
                       ...
                       <!--<label class="block text-sm">
                           ...
                           <label class="block text-sm">
                            <span class="text-gray-700 dark:text-gray-400">
                                Nombre de visites terrain
                            </span>
                            <input name="objectif_visite" type="number"  value="{{$com->objectif_visite}}"
                                class="block w-full mt-1 text-sm border-gray-600 dark:text-gray-300 dark:bg-gray-700 focus:border-red-400 focus:outline-none focus:shadow-outline-red form-input" />
                        </label>
                        
                        <label class="block text-sm">
                            <span class="text-gray-700 dark:text-gray-400">
                                Nombre de appels quotidien  (<span style=" color:red;">*</span>)
                            </span>
                            <input name="nbre_appel_quotidien" type="number"  value="{{$com->nbre_appel_quotidien}}"
                                class="block w-full mt-1 text-sm border-gray-600 dark:text-gray-300 dark:bg-gray-700 focus:border-red-400 focus:outline-none focus:shadow-outline-red form-input" />
                        </label>
