        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            @php $mois = date('m'); 
                $annee = date('Y'); 
            @endphp
            @php  $comm_sum = DB::table('commissions')->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)->whereMonth('created_at', $mois)->sum('commission'); @endphp
            Mes commissions de ce mois
             <div style="margin-left:700px;">
                    <span 
                        class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                        {{number_format(($comm_sum)) ? number_format($comm_sum) : '0'}} Fcfa
                      </span></div>
          </h2>
