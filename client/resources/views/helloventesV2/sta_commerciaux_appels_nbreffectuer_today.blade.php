        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Les prospects déjà appelé de <b>{{$commercial->prenom}} {{$commercial->nom}}
                          ...
                          @php $appels_effectuer = DB::table('suivi_prospects')->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)->whereMonth('created_at', $mois)->whereDay('created_at', $today)->get();
          ...
          <div class="w-full overflow-hidden rounded-lg shadow-xs">
              	 @if($appels_effectuer->isEmpty()) 
                 	  <p>Pas de prospect appelé ce jour</p>
					 @else
                ...
                @if($pros)
                  <tr class="text-gray-700 dark:text-gray-400">
                     ...
                     <td class="px-4 py-3 text-sm" data-toggle="tooltip" title="{{$pros->nom_entreprise}}">
                         {{ \Illuminate\Support\Str::limit(($pros->nom_entreprise) ? $pros->nom_entreprise : 'Non renseigné', 25, $end='...') }}
                    </td>
                     ...
                     @php $resulta_appel = DB::table('resultat_appels')->where('id', $entreprises->type)->first();  @endphp
                    @if($resulta_appel)
                    <td class="px-4 py-3 text-sm">
