                ...
                <div class="row col-lg-12" style="display:flex;">
                    ...
                    <div style="border-radius: 1em; width:300px; height:350px; background-color:white;border: 1px solid
                        ...
                        <div class="card-block text-center text-white">
                          ...
                          @endif
                       <br>
                    <a href="{{route('entreprise_resultat.edit', $entreprise->id)}}"> 
                            ...
                            @foreach($suivi as $suivis)
                                ...
                                <tr>
                                     ...
                                     @php $resultat = DB::table('resultat_appels')->where('id', $suivis->type)->first(); 
                                          ...
                                          $com = DB::table('commerciaus')->where('id', $suivis->commercial_suivi)->first(); 
                                     @endphp
                                    <td> 
                                    {{$suivis->choix_qualifier}}<br>
                                    @if($suivis->choix_qualifier == "Demande de dépot d'une offre de service")
                                    Date de dépôt : {{$suivis->date_depot_offre}}<br>
                                    @if($suivis->commentaire_qualifier)Commentaire : {{$suivis->commentaire_qualifier}}@endif
                                    
                                    @elseif($suivis->choix_qualifier == "Rendez-vous obtenu")
                                    Date RV : {{date('d/m/Y', strtotime($suivis->date_rendezvous))}} @if($suivis->heure_rv)à {{$suivis->heure_rv}}@endif<br>
                                    {{$suivis->personne_rv}}<br>
                                    {{$suivis->contact_rv}} {{$suivis->lieu_rv}}<br>
                                    @if($suivis->commentaire_qualifier)Commentaire : {{$suivis->commentaire_qualifier}}@endif
                                    
                                    @elseif($suivis->choix_qualifier == "Manifestation/confirmation d'interet")
                                    @if($com)Nouveau lead du prospect : {{$com->prenom}} {{$com->nom}}@endif<br>
                                    @if($suivis->commentaire_qualifier)Commentaire : {{$suivis->commentaire_qualifier}}@endif
                                    
                                    @elseif($suivis->choix_qualifier == "Dépôt d'une demande d'agrément")
                                    Date de dépôt : {{$suivis->date_depot_agreement}}<br>
                                    @if($suivis->commentaire_qualifier)Commentaire : {{$suivis->commentaire_qualifier}}@endif
                                ...
                                <tr>
                                    ...
                                    Date de dépôt : {{$suivis->raison_no_qualifier}}
                              ...
                              @if($suivis->type == 5)
                                ...
                                <tr>
                                     ...
                                     @endphp
                                    <td> 
                                    {{$suivis->choix_a_rappeler}}<br>
                                    @if($suivis->choix_a_rappeler == "Rendez-vous à confirmer")
                                    Date de relance : {{date('d/m/Y', strtotime($suivis->date_rappel))}}
                                    
                                    @elseif($suivis->choix_a_rappeler == "Indisponibilité")
                                    Date de relance : {{date('d/m/Y', strtotime($suivis->date_rappel))}}

                                    @elseif($suivis->choix_a_rappeler == "Pas de réponse")
                                    Date de relance : {{date('d/m/Y', strtotime($suivis->date_rappel))}} 
                                    
                                    @elseif($suivis->choix_a_rappeler == "Numéro incorrect / a vérifier")
                                    Date de relance : {{date('d/m/Y', strtotime($suivis->date_rappel))}} 
                                    
                                    @elseif($suivis->choix_a_rappeler == "Demande du prospect")
                                    Demande : {{$suivis->demande_rappel}} <br>
                                    Besoins : {{$suivis->besoin_rappel}} <br>
