            ...
            <div class="container">
              <div class="row d-flex justify-content-center">
                <div class="row col-lg-12" style="display:flex;">
                    ...
                    <div style="border-radius: 1em; width:300px; height:320px; background-color:white;border: 1px solid
                        ...
                        <div class="card-block text-center text-white">
                          ...
                          <i class="far fa-edit fa-2x mt-4"></i>
                            Details
                          <h4 style="color:black">Date importee : {{date('d/m/Y', strtotime($entreprise->created_at))}}</h4><br>
                          <h4 style="color:black" data-toggle="tooltip" title="{{$entreprise->secteur_activite}}">Secteur d'activite : {{ \Illuminate\Support\Str::limit(($entreprise->secteur_activite) ? $entreprise->secteur_activite : 'Non renseigné', 15, $end='...') }}</h4>
                          <br>
                          @if($entreprise->site_web)<h4 style="color:black; text-align:left">Site web : {{$entreprise->site_web}}</h4>@endif
                          <br>
                          <h4 style="color:black">Contact entreprise : {{$entreprise->tel_fixe}} / {{$entreprise->email_entreprise}}</h4>
                       <br><br>
                    <a href="{{route('prospect_produit_avendre.edit', $suivis->id)}}"> 
                    ...
                    <div style="border-radius: 1em; width:750px; height:700px; background-color:white;border: 1px solid
                        ...
                        <div class="card-block text-center text-white">
                          <i class=" fas fa-user-tie fa-7x mt-5"></i>
                          <h4 style="color:black">les résultats des appels</h4>
                          <i class="far fa-edit fa-2x mt-4"></i>
                          ...
                          <br>
                           <center>
                            <table>
                              ...
                              @if($suivis->type == 3)
                                ...
                                <tr>
                                     @php $resultat = DB::table('resultat_appels')->where('id', $suivis->type)->first(); 
                                          $domaine = DB::table('domaines')->where('id', $suivis->domaine_valider)->first(); 
                                     @endphp
                                    <td>{{date('d/m/Y', strtotime($suivis->date_rappel))}} </td>
                                    <td>{{date('d/m/Y', strtotime($suivis->created_at))}} </td>                                
                                <tr>
                                @endif
                              ...
                              @if($suivis->type == 4)
                                <tr>
                                    <th>Date rendez-vous obtenue</th>
                                     <th>Date appel </th>
                                </tr>
                                ...
                                <tr>
                                     @php $resultat = DB::table('resultat_appels')->where('id', $suivis->type)->first(); 
                                          $domaine = DB::table('domaines')->where('id', $suivis->domaine_valider)->first(); 
                                     @endphp
                                    <td>{{date('d/m/Y', strtotime($suivis->date_rendezvous))}} </td>
                                    <td>{{date('d/m/Y', strtotime($suivis->created_at))}} </td>      
                                <tr>
                                @endif
                                ...
                                @if($suivis->type == 5)
                                <tr>
                                    <th>Produits ou services à vendre</th>
                                    <th>Date appel </th>
                                </tr>
                               <tr>
                                   <td> </td> 
