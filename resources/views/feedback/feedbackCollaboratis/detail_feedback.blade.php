            ...
            <div class="page-breadcrumb">
                <div class="row align-items-center">
                    <div class="col-md-6 col-8 align-self-center">
                        <h3 class="page-title mb-0 p-0">Feedback reçu</h3>
                    </div>
										      ...
										      @foreach($feedbackdonner as $feedbackdonners)
										          <tr>
										              ...
										              @if($feedbackdonners->type_ano == 0)
                                                        <td>Anonyme</td>
                                                        @else
                                                        <?php $agents = DB::table('agents')->select('users.photo')->join('users', 'agents.user_id', 'users.id')->where('id', $feedbackdonners->agent_id)->first(); 
                                                           ?>
                                                         <td>{{$agents->prenom}} {{$agents->nom}}</td>
                                                        @endif
                                                        <td class="d-flex align-items-center">
                                                       @if($agents->photo == NULL)
                                                       <span class="rounded-circle" style="border: 1px solid #3f6ad8; background: white; color:#2C365E; font-weight:bold; font-size:18px; padding:10px; text-shadow: 1px 1px 2px white;">{{substr($agents->prenom, 0,1)}} {{substr($agents->nom, 0,1)}}</span>
                                                       @else
                        						      	<img width="50" height="50" class="rounded-circle" src="{{ url('images/', $agents->photo) }}" alt="">
                        						       @endif
                        						    </td>
                                                        
                        						    </td>
										               
										              <td>{{ ($feedbackdonners->apprecier && $feedbackdonners->crypted) ? decrypt($feedbackdonners->apprecier) : ''}}</td>
										              <td>{{ ($feedbackdonners->ameliorer && $feedbackdonners->crypted) ? decrypt($feedbackdonners->ameliorer) : ''}}</span></td>
										          </tr>
										      @endforeach
										      </tbody>
						</table>
