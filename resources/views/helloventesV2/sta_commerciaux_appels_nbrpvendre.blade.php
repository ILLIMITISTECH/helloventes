...
<body>
       ...
       <a data-toggle="tooltip" title="Retour" style="width:90px; margin-left:30px"  type="button" id="PopoverCustomT-1" class="nm" href="javascript:history.go(-1)" >
        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Demande de produits ou services à vendre de <b>{{$commercial->prenom}} {{$commercial->nom}}
          </h2>
                          @php $produit_avendre = DB::table('suivi_prospects')->where('type', 5)->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)->whereMonth('created_at', $mois)->get(); @endphp
          ...
          <div class="w-full overflow-hidden rounded-lg shadow-xs">
              	 @if($produit_avendre->isEmpty()) 
                 	  <p>Pas de demande de produits ou services à vendre </p>
					 @else
              ...
              <table class="w-full whitespace-no-wrap">
                <thead>
                  <tr
                    ...
                    <!--<th class="px-4 py-3">Date de rendez-vous</th>-->
                    <th class="px-4 py-3">Options</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                @foreach($produit_avendre as $entreprises) 
                  <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-sm">
                     ...
                     <td class="px-4 py-3 text-sm">
                      {{($pros->email_entreprise) ? $pros->email_entreprise : ''}} / {{($pros->tel_fixe) ? $pros->tel_fixe : ''}}
                    </td>
                    <!-- <td class="px-4 py-3 text-sm">-->
                    <!--  {{($entreprises->date_rendezvous) ? $entreprises->date_rendezvous : ''}}-->
                    <!--</td>-->
 ...
 <a href="{{ route('fiche_prospect_produit_avendre',$entreprises->id) }}">
                      ...
                      <path d="M3.654 1.328a.678.678 0 0 0-1.015-.063L1.605 2.3c-.483.484-.661 1.169-.45 1.77a17.568 17.568 0 0 0 4.168 6.608 17.569 17.569 0 0 0 6.608 4.168c.601.211 1.286.033 1.77-.45l1.034-1.034a.678.678 0 0 0-.063-1.015l-2.307-1.794a.678.678 0 0 0-.58-.122l-2.19.547a1.745 1.745 0 0 1-1.657-.459L5.482 8.062a1.745 1.745 0 0 1-.46-1.657l.548-2.19a.678.678 0 0 0-.122-.58L3.654 1.328zM1.884.511a1.745 1.745 0 0 1 2.612.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.678.678 0 0 0 .178.643l2.457 2.457a.678.678 0 0 0 .644.178l2.189-.547a1.745 1.745 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.634 18.634 0 0 1-7.01-4.42 18.634 18.634 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877L1.885.511zm10.762.135a.5.5 0 0 1 .708 0l2.5 2.5a.5.5 0 0 1 0 .708l-2.5 2.5a.5.5 0 0 1-.708-.708L14.293 4H9.5a.5.5 0 0 1 0-1h4.793l-1.647-1.646a.5.5 0 0 1 0-.708z"/>
                         ...
                         <span style="margin-left:3px; height:28px;"  class="px-2 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" tabindex="0" style="margin-right : 5%;" data-toggle="tooltip" title="Mettre à jour le prospect">
                          <a type="submit" id="PopoverCustomT-1" class="btn btn-primary" href="{{route('prospect_produit_avendre.edit', $entreprises->id)}}" >
