                ...
                <div class="row col-lg-12" style="display:flex;">
                    ...
                    <div style="border-radius: 1em; width:300px; height:350px; background-color:white;border: 1px solid
                        ...
                        <div class="card-block text-center text-white">
                          ...
                          @endif
                       <br>
                    <a href="{{route('entreprise_rv.edit', $entreprise->id)}}"> 
                            ...
                            <table>
                                ...
                                <tr>
                                   ...
                                   <td> 
                                    ...
                                    @if($suivis->choix_qualifier == "Rendez-vous obtenu")
                                    Date RV : {{date('d/m/Y', strtotime($suivis->date_rendezvous))}} @if($suivis->heure_rv)à {{$suivis->heure_rv}}@endif<br>
                                    @if($suivis->lieu_rv)Lieu RV: {{$suivis->lieu_rv}}<br>@endif
                                    @if($suivis->personne_rv){{$suivis->personne_rv}}@endif<br>
                                    @if($suivis->contact_rv){{$suivis->contact_rv}}@endif
                                ...
                                @if($suivis->statut_rv == 1)
                                <tr>
                                    <th>Résultat du rendez-vous</th>
                                    <th></th>
                                    </tr>
                                    <tr>
                                    <td>{{($suivis->resultat_rv) ? $suivis->resultat_rv : 'Pas de résultats'}} </td>
                                    <td>
                                    <span style="margin-left:3px; height:28px;"  class="px-2 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" tabindex="0" style="margin-right : 5%;" >
                                      <a type="submit" id="PopoverCustomT-1" class="btn btn-primary" href="{{route('action_rv.voir', $suivis->id)}}" >
                                          Voir les actions
                                        </a>
                                    </span>
                                    </td>
