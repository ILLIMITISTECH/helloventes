...
<body>
       ...
       <a data-toggle="tooltip" title="Retour" style="width:90px; margin-left:30px"  type="button" id="PopoverCustomT-1" class="nm" href="javascript:history.go(-1)" >
        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Les prospects déjà appelé de <b>{{$commercial->prenom}} {{$commercial->nom}}
          </h2>
                          @php 
        ...
        <div class="w-full overflow-x-auto">    
              <table class="w-full whitespace-no-wrap">
                ...
                @php $prospect_appel = DB::table('prospect_a_appellers')
                            ->where('id', $entreprises->prospect_appel_id)->first(); @endphp
                  <tr class="text-gray-700 dark:text-gray-400">
                   
                     <td class="px-4 py-3 text-sm" data-toggle="tooltip" title="{{$prospect_appel->nom_entreprise}}">
                         {{ \Illuminate\Support\Str::limit(($prospect_appel->nom_entreprise) ? $prospect_appel->nom_entreprise : 'Non renseigné', 25, $end='...') }}
                    </td>
                    
                    
                     <td class="px-4 py-3 text-sm" data-toggle="tooltip" title="{{$prospect_appel->secteur_activite}}">
                         {{ \Illuminate\Support\Str::limit(($prospect_appel->secteur_activite) ? $prospect_appel->secteur_activite : 'Non renseigné', 25, $end='...') }}
                    </td>
                   
                   
                    
                     <td class="px-4 py-3 text-sm">
                      {{($prospect_appel->email_entreprise) ? $prospect_appel->email_entreprise : ''}} / {{($prospect_appel->tel_fixe) ? $prospect_appel->tel_fixe : ''}}
                    </td>
                   @php $types = DB::table('suivi_prospects')->where('prospect_appel_id', $entreprises->id)->where('type', '!=', NULL)->orderby('id', 'desc')->first(); @endphp
                    @if($types)
                    ...
                    <div style="display:flex ">
                        ...
                        <a href="{{ route('fiche_prospect_resultat',$entreprises->id) }}">
  ...
  <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533L8.93 6.588zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"/>
                    ...
                    <!--     <span style="margin-left:3px; height:28px;"  class="px-2 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" tabindex="0" style="margin-right : 5%;" data-toggle="tooltip" title="Resultats appels">-->
