...
<br>
                          ...
                          <h4 style="color:black; text-align:left" data-toggle="tooltip" title="{{$entreprise->secteur_activite}}">Secteur d'activite : {{($entreprise->secteur) ? $entreprise->secteur : 'Non renseigné'}}</h4>
                          <br>
                          @if($entreprise->site_web)<h4 style="color:black">Site web : {{$entreprise->site_web}}</h4>@endif
                          <br>
                          @if($entreprise->pays)<h4 style="color:black; text-align:left">Pays : {{$entreprise->pays}}</h4>@endif
                          
                       <br>
                    <a href="{{route('bdd_prospects.edit', $entreprise->id)}}"> 
                          ...
                          <br>
                           <center>
                            <table>
                                ...
                                <tr>
                                    <td>{{($entreprise->email_entreprise) ? $entreprise->email_entreprise : '-'}}</td>
                                    <td>{{($entreprise->taille_prospect) ? $entreprise->taille_prospect : '-'}} </td>
                                    <td>{{($entreprise->type_prospect) ? $entreprise->type_prospect : '-'}} </td>
                                    <td>{{($entreprise->prenom_contact2) ? $entreprise->prenom_contact2 : '-'}} {{($entreprise->nom_contact2) ? $entreprise->nom_contact2 : '-'}}</td>
                                    <td>{{($entreprise->email_contact2) ? $entreprise->email_contact2 : '-'}} <br> {{($entreprise->mobile_contact2) ? $entreprise->mobile_contact2 : '-'}}</td>
