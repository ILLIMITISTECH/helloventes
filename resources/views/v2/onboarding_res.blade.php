        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
           ...
           <div style = "margin-left: 600px; margin-top:-50px" > 
                                 <form action="{{route('onboarding_search_par_com_res')}}" method="get" style="margsearchfin-top:5px; display:flex;">
                                    <select name="searchCommerciau" style="width:220px;height:40px"  id="statut" style="margin-right:10px; display:flex;">
                                        ...
                                        $commerciauxs = DB::table('users')->where('superieur_id', $moi->id)->get();
                                        
                                        @endphp
                                        
                                        @foreach($commerciauxs as $result_comer)
                                        <option value="{{$result_comer->id}}">{{$result_comer->prenom}} {{$result_comer->nom}}</option>
                                        @endforeach
             ...
             <h6> 
              ...
              @endif
                                        ...
                                        @php $dnow = date('d'); $dnowH = date('H:i'); $hr = date('H'); $mnt = date('i') + 20; @endphp 
                                            ...
                                            ->where('email', '!=', 'admin@gmail.com')->where('superieur_id', $moi->id)
                                            ->count(); 
                                            @endphp
                                            @php $nbruserss = DB::table('users')->where('email', '!=', 'admin@gmail.com')->where('superieur_id', $moi->id)
