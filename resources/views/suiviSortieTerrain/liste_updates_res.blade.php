       ...
       <a data-toggle="tooltip" title="Retour" style="width:90px; margin-left:30px"  type="button" id="PopoverCustomT-1" class="nm" href="javascript:history.go(-1)" >
        ...
        <div class="container px-6 mx-auto grid">
          ...
          <div  class="col-md-3" style = "margin-top:-20px" align="right" >
                                 <form action="{{route('filtrer_liste_updates_res')}}" method="get" >
          ...
          <div class="w-full overflow-hidden rounded-lg shadow-xs">
            ...
            <div class="w-full overflow-x-auto">
              <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                @foreach($updates_pole as $updatess_pole) 
                  <tr class="text-gray-700 dark:text-gray-400">
                      
                      @php $commer = DB::table('commerciaus')->where('id', $updatess_pole->commercial_id)->first(); @endphp
                     @if($commer)
                    <td class="px-4 py-3 text-sm">
                      {{ strtoupper(($commer->prenom) ? $commer->prenom : '')}} {{ strtoupper(($commer->nom) ? $commer->nom : '')}}
                    </td>
                    ...
                    @endif
                    @php $prospect = DB::table('prospects')->where('id', $updatess_pole->prospect_id)->first();
                     $opportunites = DB::table('opportunites')->where('id', $updatess_pole->opportunite_id)->first(); @endphp
                    <td align="left" data-toggle="tooltip" title="{{$opportunites->libelle}}">
                   ...
                   <br>
                    ...
                    @if($updatess_pole->libelle)
                    <td class="px-4 py-3 text-sm" data-toggle="tooltip" title="{{$updatess_pole->libelle}}">
                    {{ \Illuminate\Support\Str::limit($updatess_pole->libelle, 25, $end='...') }}
                    </td>
                     @else
                    <td class="px-4 py-3 text-sm">-</td>
                    @endif
                     <td class="px-4 py-3 text-sm">
                    {{date('d/m/Y', strtotime($updatess_pole->date))}}
                    </td>
                    <td class="px-4 py-3 text-sm">
                    {{($updatess_pole->personne) ? $updatess_pole->personne : '--'}}
                    </td>
                    @if($updatess_pole->commentaire)
                    <td class="px-4 py-3 text-sm" data-toggle="tooltip" title="{{$updatess_pole->commentaire}}">
                        {{ \Illuminate\Support\Str::limit($updatess_pole->commentaire, 25, $end='...') }}
                    </td>
                    @else
                    <td class="px-4 py-3 text-sm">-</td>
                    @endif
