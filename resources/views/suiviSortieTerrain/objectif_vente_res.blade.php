              ...
              <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                 ...
                 @foreach($stock_mensuelles as $stock_mensuelle)
                    @php 
                       setlocale(LC_TIME, 'fr_FR'); 
                      $commerciauxf = DB::table('commerciaus')->where('id', $stock_mensuelle->commercial_id)->where('superieur_id', $moi->id)->orderBy('prenom', 'desc')->paginate();
                    @endphp
            ...
            Objectifs et ventes de ma team
                         ...
                         <div  class="col-md-3" style = "margin-top:-20px" align="right" >
                              @php
                                             ...
                                             $datnow = date('m');
                                        @endphp
                                 <form action="{{route('search_par_com_res')}}" method="get" >
                            ...
                            <br>
          <div class="w-full overflow-hidden rounded-lg shadow-xs">
              	 @if($stock_mensuelles_pole->isEmpty()) 
                 	  <p>Pas de commerciaux</p>
					 @else
            <div class="w-full overflow-x-auto">
              <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                 ...
                 @foreach($stock_mensuelles_pole as $stock_mensuelle_pole)
                    @php 
                       setlocale(LC_TIME, 'fr_FR'); 
                      $commerciauxf = DB::table('commerciaus')->where('id', $stock_mensuelle_pole->commercial_id)->orderBy('prenom', 'desc')->paginate();
                    @endphp
               @foreach($commerciauxf as $commerciau) 
                @php 
                       $photo = DB::table('users')->where('id', $commerciau->user_id)->first();
                    @endphp
                @php $com_pourcentage = DB::table('parametres')->where('commission', str_replace(array("\r\n", "\r", "\n", "\t"," "), '', $stock_mensuelle_pole->commission_p))->first(); @endphp
                  <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-sm">
                       {{ucfirst(strftime('%B', strtotime($stock_mensuelle_pole->created_at)))}} /  {{date('Y', strtotime($stock_mensuelle_pole->created_at))}}
                    ...
                    <span
                        class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                        {{($stock_mensuelle_pole->objectif_mois) ?  number_format($stock_mensuelle_pole->objectif_mois) : 0}}
                      </span>
                    </td>
                    ...
                    <span
                        class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                        {{intval($stock_mensuelle_pole->pourcentage)}}%
                      </span>
                    </td>
