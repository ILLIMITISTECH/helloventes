...
<br>
        ...
        <div class="container px-6 mx-auto grid">
            ...
            Rapport <b style="color:#9045e2">mensuel</b> de performance commerciale : Team CI
          </h2>
           @php $chtoday = date('m'); $rap_check = DB::table('performance_ci_globales')->whereMonth('created_at', $chtoday)->orderBy('id', 'DESC')->first(); @endphp
            @if($rap_check)
           @php DB::table('performance_ci_globales')->whereMonth('created_at', $chtoday)
           ...
           <br><br>
                      <div  class="col-md-3" style = "margin-top:-20px" align="right" >
                              @php
                                             ...
                                             $year_mensuelles = DB::table('performance_ci_globales')->whereYear('created_at', $this_year)->whereMonth('created_at', $les_moisf)->pluck('created_at')->toArray();
                                         ...
                                         $month_not_here = array("01","02","03","04","05","06","7","8","9","10","11","12");
                                            ...
                                            $result_month_not_heres = array_diff($month_not_here, $result_mois);
                                             $datnow = date('m');
                                        @endphp
                                 <form action="{{route('rapport_teamCIFiltre')}}" method="get" >
                                      ...
                                      <select name="searchMois" style="width:220px;height:40px"  style="margin-right:10px; display:flex;">
                                        ...
                                        <option value="" disabled selected>Rechercher par mois</option>
                                          ...
                                          @php  $obje_mensuelle = DB::table('performance_ci_globales')->whereMonth('created_at', $result_moi)->first(); @endphp
 <!--@if(date('m', strtotime($obje_mensuelle->created_at)) == 12)-->
                                         ...
                                         @if(date('m', strtotime($obje_mensuelle->created_at)) == "12")
                                         <option value="{{$result_moi}}">{{ucfirst(strftime('Décembre / %Y', strtotime($obje_mensuelle->created_at)))}}</option>
                                           @elseif(date('m', strtotime($obje_mensuelle->created_at)) == "2")
                                         <option value="2">{{ucfirst(strftime('Février / %Y', strtotime($obje_mensuelle->created_at)))}}</option>
                                          @elseif(date('m', strtotime($obje_mensuelle->created_at)) == "8")
            ...
            @endphp
          @php
                            $segment = request()->input('searchMois');
          ...
          @if($segment)
           <p style="font-size:20px">
               @if($segment == 1)
               <h5 class="text-primary">JANVIER</h5>
               @elseif($segment == 2)
               <h5 class="text-primary">FEVRIER</h5>
               @elseif($segment == 3)
               <h5 class="text-primary">MARS</h5>
               @elseif($segment == 4)
               <h5 class="text-primary">AVRIL</h5>
               @elseif($segment == 5)
               <h5 class="text-primary">MAI</h5>
               @elseif($segment == 6)
               <h5 class="text-primary">JUIN</h5>
               @elseif($segment == 7)
               <h5 class="text-primary">JUILLET</h5>
               @elseif($segment == 8)
               <h5 class="text-primary">AOUT</h5>
               @elseif($segment == 9)
               <h5 class="text-primary">SEPTEMBRE</h5>
               @elseif($segment == 10)
               <h5 class="text-primary">OCTOBRE</h5>
               @elseif($segment == 11)
               <h5 class="text-primary">NOVEMBRE</h5>
               @elseif($segment == 12)
               <h5 class="text-primary">DECEMBRE</h5>
               @endif
               </p>
          @else
          @if($per)
          <p style="font-size:20px">{{ucfirst(strftime('%B %Y', strtotime($thifordate)))}}</p>
          @endif
          @endif
