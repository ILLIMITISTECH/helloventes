      ...
      <main class="h-full pb-16 overflow-y-auto">
        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Rapport des commerciaux de ce mois
          </h2>
            ...
            <div class="w-full overflow-x-auto">
              <table class="w-full whitespace-no-wrap">
                ...
                @foreach($commerciaux as $commerciau) 
                                @php 
                                    $opportunite = DB::table('opportunites')->where('commercial_id', $commerciau->id)->where('archiver', 0)
                                    ->whereMonth('created_at', $action_mois)
                                    ->whereYear('created_at', $annee)->count(); 
                                    $prospect = DB::table('prospects')->where('commercial_id', $commerciau->id)
                                    ->whereMonth('created_at', $action_mois)
                                    ->whereYear('created_at', $annee)->count();
                                ...
                                @endphp
                  @if($commerciau->nom_role == "responsable" or $commerciau->nom_role == "commerciaux")
                  <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-sm">
                        <a href="{{ route('prospect_comIndivuduel.maTeam',$commerciau->id) }}" data-toggle="tooltip" title="Voir les prospects de {{$commerciau->prenom}}">
                    ...
                    <a href="{{ route('opportunite_comIndividuel.maTeam',$commerciau->id) }}" data-toggle="tooltip" title="Voir les opportunités de {{$commerciau->prenom}}">
                      <span
                        class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                        {{$opportunite}}
                      </span></a>
