       ...
       <a data-toggle="tooltip" title="Retour" style="width:90px; margin-left:30px"  type="button" id="PopoverCustomT-1" class="nm" href="javascript:history.go(-1)" >
        ...
        <div class="container px-6 mx-auto grid">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            ...
            $vente_sumFirst = DB::table('ventes')->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)
            ->whereMonth('created_at', '>=', 01)->whereMonth('created_at', '<=', 02)->sum('montant'); 
            $vente_sumSecond = DB::table('ventes')->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)
            ->whereMonth('created_at', '>=', 04)->whereMonth('created_at', '<=', 06)->sum('montant'); 
            $vente_sumThirt = DB::table('ventes')->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)
            ->whereMonth('created_at', '>=', 7)->whereMonth('created_at', '<=', 9)->sum('montant'); 
            $vente_sumFour = DB::table('ventes')->where('commercial_id', $commercial->id)->whereYear('created_at', $annee)
            ->whereMonth('created_at', '>=', 10)->whereMonth('created_at', '<=', 12)->sum('montant'); 

            @endphp
            Mes ventes de ce trimestre
             <div style="margin-left:450px;">
                    
                    <!--@if( ($trimestre >= 01) && ($trimestre <= 03) )-->
                    <!--  <span -->
                    <!--    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">-->
                    <!--    {{number_format(($vente_sumFirst)) ? number_format($vente_sumFirst) : '0'}} Fcfa-->
                    <!--  </span>-->
                    <!--@endif-->
                    ...
                    <!--  <span -->
                    <!--    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">-->
                    <!--    {{number_format(($vente_sumSecond)) ? number_format($vente_sumSecond) : '0'}} Fcfa-->
                    <!--  </span>-->
                    <!--@endif-->
                    ...
                    <!--  <span -->
                    <!--    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">-->
                    <!--    {{number_format(($vente_sumThirt)) ? number_format($vente_sumThirt) : '0'}} Fcfa-->
                    <!--  </span>-->
                    <!--@endif-->
                    ...
                    <!--  <span -->
                    <!--    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">-->
                    <!--    {{number_format(($vente_sumFour)) ? number_format($vente_sumFour) : '0'}} Fcfa-->
                    <!--  </span>-->
                    <!--@endif-->
            </div>
          </h2>
