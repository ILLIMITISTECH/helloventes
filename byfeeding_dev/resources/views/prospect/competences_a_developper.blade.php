    ...
    <h5>
      ...
      <div class="row">
        <div class="col-lg-10">
          ...
          <div class="card">
            <div class="card-body">
                  @php $suivi = DB::table('suivi_agent_prospects')->where('agent_id', $agent->id)->OrderBy('id' , 'desc')->first(); @endphp
                 
                                <form action="{{route('competences.a_developper', $suivi->id)}}" method="post" class="form-horizontal" id="demo1-upload" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                            ...
                                            <div class="row mb-3">
                                              <label for="inputText" class="col-sm-12 col-form-label">Quelles sont les 3 compétences clés  que vous souhaitez développer au cours des 6 prochains mois, au regard des résultats du feedback 360, et de votre plan d'actions de développement personnel ? </label>
                                             <br><br>
                                              <div class="col-sm-4"> <div align="right"><b style="color:red;">*</b></div>
                                              <select class="form-select" name="competence_dev1" aria-label="Default select example"><b style="color:red;">*</b>
                                                    <option value="0" selected>Compétence 1  </option>
                                                      @foreach($competences as $competence)  
                                                          <option value="{{$competence->id}}">{{$competence->libelle}}</option>
                                                      @endforeach
                                              </select>
                                              </div>
                                              <div class="col-sm-4"><div align="right"><b style="color:white;">*</b></div>
                                              <select class="form-select" name="competence_dev2" aria-label="Default select example">
                                                    <option value="0" selected>Compétence 2</option>
                                                      @foreach($competences as $competence)  
                                                          <option value="{{$competence->id}}">{{$competence->libelle}}</option>
                                                      @endforeach
                                              </select>
                                              ...
                                              <select class="form-select" name="competence_dev3" aria-label="Default select example">
                                                    <option  value="0" selected>Compétence 3</option>
                                                      @foreach($competences as $competence)  
                                                          <option value="{{$competence->id}}">{{$competence->libelle}}</option>
                                                      @endforeach
                                              </select>
                                ...
                                <h5 class="card-title">Rappel des compétences que vos collègues vous suggèrent de développer :</h5>
              ...
              <script>
                document.addEventListener("DOMContentLoaded", () => {
                  new ApexCharts(document.querySelector("#barChart"), {
                    series: [{
                      data: [{{$competence_1}},{{$competence_2}},{{$competence_5}},{{$competence_3}},{{$competence_4}},{{$competence_6}},{{$competence_7}},{{$competence_8}},{{$competence_9}},{{$competence_10}},{{$competence_11}}]
                    }],
                    chart: {
                      type: 'bar',
                      height: 550
                    ...
                    xaxis: {
                      categories: ['Leadership', 'Management', 'Confiance en soi', 'Délégation', 'Écoute active','Gestion de conflits','Techniques de Vente et négociation','Prise de parole en public','Travail en équipe','Intelligence émotionnelle','Focus et productivité'
                      ],
                    }
                  }).render();
                });
