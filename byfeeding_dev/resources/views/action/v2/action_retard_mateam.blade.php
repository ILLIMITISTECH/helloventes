            ...
            <div class="page-breadcrumb">
                <div class="row align-items-center">
                    <div class="col-md-6 col-8 align-self-center">
                        <h3 class="page-title mb-0 p-0">Les actions en retard </h3>
                    </div>
                                        ...
                                        <thead>
                                            ...
                                            <tbody>
                                                 ...
                                                 @php
                                                  ...
                                                  $action_mois = date('m');
                                                  $agentMe = DB::table('agents')->where('user_id', Auth::user()->id)->first();
                                                  $agentTeams = DB::table('agents')->where('direction_id', $agentMe->direction_id)->get();
                                                  $projets_pros = array();
                                                foreach($agentTeams as $agentTeam)
                                                {
                                                if($agentTeam)
                                                {
                                                 $projets_pross = DB::table('projets')->where('responsable', $agentTeam->id)->get();
                                                 
                                                 array_push($projets_pros, $projets_pross);
                                                 }
                                                }
                                               
                                                @endphp 
                                        
                                            @foreach($projets_pros as $proj)  
                                             @foreach($proj as $pro)  
                                              ...
                                              $pro_actions = DB::table('actions')->where('projet_id', $pro->id)->get();
                                                
                                                @endphp   
                                                @foreach($pro_actions as $pro_action) 
                                               
                                               @if(($pro_action->deadline < now()) && $pro_action->pourcentage < 100)
                                                @if($pro_action->visibilite == 0) 
                                                 <tr class = "actions">
                                                @if($pro_action)
                                                <td class="align-middle"></td>
                                                                    <td class="align-middle" data-toggle="tooltip" title="{{$pro_action->libelle}}"> {{ \Illuminate\Support\Str::limit($pro_action->libelle, 35, $end='...') }}</td>
                                                     @php $agentbackups = DB::table('agents')->where('id', $pro_action->agent_id)->first(); @endphp
                                                    <td class="align-middle" style="width:50px;"><span class="round">{{substr($agentbackups->prenom, 0, 1)}} {{substr($agentbackups->nom, 0, 1)}}</span></td>
                                                    ...
                                                    @endif
                                                                    ...
                                                                    @endif
                                                                        ...
                                                                        <div class="student-dtl" style="display:flex;">
                                                                                    ...
                                                                                    <span class="d-inline-block" tabindex="0" style="margin-right : 10%;" data-toggle="tooltip" title="Mettre à jour votre action">
                                                                                        <a type="submit" id="PopoverCustomT-1" class="btn btn-primary" href="{{route('action_ma_teamR.editer', $pro_action->id)}}" >
                                                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-repeat" viewBox="0 0 16 16">
                                                                                              <path d="M11.534 7h3.932a.25.25 0 0 1 .192.41l-1.966 2.36a.25.25 0 0 1-.384 0l-1.966-2.36a.25.25 0 0 1 .192-.41zm-11 2h3.932a.25.25 0 0 0 .192-.41L2.692 6.23a.25.25 0 0 0-.384 0L.342 8.59A.25.25 0 0 0 .534 9z"/>
                                                                                              <path fill-rule="evenodd" d="M8 3c-1.552 0-2.94.707-3.857 1.818a.5.5 0 1 1-.771-.636A6.002 6.002 0 0 1 13.917 7H12.9A5.002 5.002 0 0 0 8 3zM3.1 9a5.002 5.002 0 0 0 8.757 2.182.5.5 0 1 1 .771.636A6.002 6.002 0 0 1 2.083 9H3.1z"/>
                                                                                            </svg>
