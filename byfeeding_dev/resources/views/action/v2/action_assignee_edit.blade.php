                ...
                <div class="app-main__outer">
                    <div class="app-main__inner">
                     <div class="main-card mb-3 card">
                            <div class="card-body">
                                <form action="{{route('action_assignee.update', $action->id)}}" method="post"  class="dropzone dropzone-custom needsclick add-professors" id="demo1-upload" enctype="multipart/form-data">
                                                    @method('PATCH')
                                                     @csrf
                                            <div class="tab-pane" id="tab1">
                                             ...
                                             <fieldset>
                                                    ...
                                                    <div class="control-group">
                                                         <label class="control-label" for="focusedInput">Sélectionner le Responsable</label>
                                                      <div class="controls">
                                                          @php $agentrespon = DB::table('agents')->where('id', $action->agent_id)->first(); @endphp
                                                         <select name="agent_id" class="form-control">
                                                          <option value="{{$action->agent_id}}">Responsable : {{$agentrespon->prenom}} {{$agentrespon->nom}}</option>
                                                   ...
                                                   <div class="control-group">
                                                      ...
                                                      <div class="controls">
                                                           @php $agentbakup = DB::table('agents')->where('id', $action->bakup)->first(); @endphp
                                                         <select name="bakup" class="form-control">
                                                            @if($agentbakup)
                                                                <option value="{{$action->bakup}}">Backup : {{$agentbakup->prenom}} {{$agentbakup->nom}}</option>
                                                             @else
                                                                <option value="{{$action->bakup}}">Sélectionner</option>
                                                            @endif
                                                          @foreach($agents as $agent)
                                                          <option value="{{$agent->id}}">{{$agent->prenom}} {{$agent->nom}}</option>
                                                          @endforeach                                    
