            ...
            <div class="page-breadcrumb">
                <div class="row align-items-center">
                    <div class="col-md-6 col-8 align-self-center">
                        <h3 class="page-title mb-0 p-0">Les Actions Assignées</h3>
                    </div>
                                    ...
                                    <table class="table stylish-table no-wrap">
                                        ...
                                        @foreach($actions as $action)  
                                            <tr>
                                                ...
                                                @php $agentresp = DB::table('agents')->where('id', $action->agent_id)->first(); @endphp
                                                @if($agentresp)
                                                <td class="align-middle"><style="width:50px;"><span class="round">{{substr($agentresp->prenom, 0, 1)}} {{substr($agentresp->nom, 0, 1)}}</span></td>
                                                @else
                                            ...
                                            <div class="student-dtl" style="display:flex;">
                                                       @php if($agentresp)  $respon = $agentresp->email; @endphp
                                                       @php if($agentbackup) $back = $agentbackup->email;   @endphp
                                                <span class="d-inline-block" tabindex="0" style="margin-right : 10%;" data-toggle="tooltip" title="Modifier votre action">
                                                   <form action="/sendmailrappel" method="post" id="target" class="form">
                                                       <input type="hidden" value="{{csrf_token()}}" name="_token"/>
                                                       <input type="hidden" name="name" value="{{$action->prenom}}">
                                                       <input type="hidden" name="email" value="{{$respon}}">
                                                       @isset($back)
                                                       <input type="hidden" name="emails" value="{{$back}}">
                                                        @endisset
                                                       <input type="hidden" name="message" value="L'action : {{$action->libelle}} est priotitaire ">
                                                       <button type="submit" class="btn btn-primary" style="margin-left: 5px;">Rappel</button>
                                                    </form>
                                                  </span>   
                                                    <span class="d-inline-block" tabindex="0" style="margin-right : 10%;" data-toggle="tooltip" title="Modifier votre action">
                                                        <a type="submit" id="PopoverCustomT-1" class="btn btn-primary"  href="{{route('action_assignee.edit', $action->id)}}" >
